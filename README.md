...
| <a name="input_global_image_pull_secrets"></a> [global\_image\_pull\_secrets](#input\_global\_image\_pull\_secrets) | List of names of imagePullSecrets to use for pulling agent or cluster shield images. These secrets are used only if `agent_image_pull_secrets` or `cluster_shield_image_pull_secrets` are not specified. If no value is provided, container images must be public otherwise kubelet will not be able to pull them. | `list(string)` | `[]` | no |
